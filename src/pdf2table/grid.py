...
class GridMiner:
    ...
    def reset_rownum(page_data: pd.DataFrame) -> pd.DataFrame:
        ...
        return page_data
    #
    def delete_blank_lines(self) -> pd.DataFrame:
        page_data = self.tesseract_page_data.copy()
        rownum_non_blank_lines = []
        # Går gjennom radnummer for radnummer for å se om finner
        for rownum_value in page_data['rownum'].dropna().unique():
            rownum_text_values = page_data.query(f"conf > 0 and rownum == {rownum_value}")['text'].to_list()
            for rownum_text_value in rownum_text_values:
                if re.search(pattern = r'\S',string = rownum_text_value) is not None:
                    rownum_non_blank_lines.append(int(rownum_value))
                    break
                #
            #
        #
        page_data_non_empty_lines = page_data.query(f"rownum in {rownum_non_blank_lines}")
        return page_data_non_empty_lines
    #
    ...
    def extract_table_area(self,reset_row_index: bool = True) -> pd.DataFrame:
        page_data_no_blank_lines = self.delete_blank_lines()
        page_data = self.reset_rownum(page_data_no_blank_lines)
